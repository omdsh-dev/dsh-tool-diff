...
This plugin has been migrated to the DSH 0.1.2-alpha.2 dependency line and fully verified end-to-end in an isolated consumer of `local harness 0.1.2-alpha.2`:
...
- **Consumption verification**: tarball loaded into a 0.1.2-alpha.2 consumer → `dsh --profile compat --dump-config` shows this plugin's row → the tool genuinely registers and executes
- **Launch method**: local harness checkout (GitHub `dsh-v0.1.2-alpha.2`) `dsh web`; npm release line via `npx -p @deepseek-ai/dsh@next dsh web` (lib production mode; don't `install -g` globally)
...
As of DSH 0.1.2-alpha.2, this plugin can be installed into any profile as a standalone bundle in one step (repositories live at https://github.com/omdsh-dev, public):
